...
@implementation startScreen{
    ...
    NSTimer * timer;
    UIViewController * selectorViewController;
//    NSString * directory;
...
- (void)viewDidLoad
{
    ...
    NSArray *extensions = [NSArray arrayWithObjects:@"json", nil];
...
- (IBAction)continueButtonPressed:(id)sender {
    [timer invalidate];
    selectorViewController * sel = [[selectorViewController alloc]init];
    [self presentModalViewController:sel animated:NO];
}
...
- (IBAction)clearButtonPressed:(id)sender {
    ...
    NSArray *extensions = [NSArray arrayWithObjects:@"json", @"jpg", nil];
    NSArray * dirContents = [filemgr contentsOfDirectoryAtPath:qListPath error:nil];
    
    //NSLog(@"docDir: %@",docDir);
    //NSLog(@"dir contents: %@",dirContents2);
    NSArray * onlyTXTs = [dirContents filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"pathExtension IN %@", extensions]];
    ...
    [self updateLinkStatus:timer];
    [clearButton setTitle:@"cleared" forState:UIControlStateNormal];
...
- (IBAction)downloadDirChangePressed:(id)sender {
    ...
    downloadDirectory = [NSString stringWithFormat:@"/%@",downloadDirText.text];
...
- (void)restClient:(DBRestClient*)client loadedMetadata:(DBMetadata*)metadata {
    ...
    if (uploading){
        ...
        NSArray *extensions = [NSArray arrayWithObjects:@"png",@"txt",nil];
    ...
    if (downloading){
        ...
        if(downloadCount == 0){
            ...
            [clearButton setTitle:@"clear downloads" forState:UIControlStateNormal];
            
            [appDelegate.allQnsAndPaths addObjectsFromArray:newQuestions];
...
- (void)restClient:(DBRestClient *)client loadMetadataFailedWithError:(NSError *)error {
    ...
    [NSThread sleepForTimeInterval:1];   //Wait a sec, then try again.  Maybe it would be better to unlink, then re-link?
...
- (void)restClient:(DBRestClient*)client uploadFileFailedWithError:(NSError *)error{
    ...
    //errorLabel.text = error.localizedDescription;  //No need to panic the user with this error?
...
- (void)restClient:(DBRestClient*)client loadedFile:(NSString*)localPath {
    ...
    if ([[components objectAtIndex:1] isEqualToString:@"json"]){
        [newQuestions addObject:filenamePath];
    }
    ...
    if(downloadCount == 0){
        ...
        [clearButton setTitle:@"clear downloaded questions" forState:UIControlStateNormal];
